...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        print ("\n legal:\t")
        print legal
        print ("\nlivingGhostPositionDistrbution:\t")
        print livingGhostPositionDistributions
        
        # condensed initial state
        pacMan Position:        
        (1, 6)
        ...
        self.ghostBeliefs:      
        [{(7, 3): 0.10829103214890017, ......, (8, 4): 0.10829103214890017}, {.......}]
        
        legal:         
        ['Stop', 'East', 'South']

        livingGhostPositionDistrbution: 
        [{(7, 3): 0.10829103214890017, ......, (8, 4): 0.012587296767604271}, {......}]
        ...
        for i in livingGhosts:
            if i is True:
                g_dstr = livingGhostPositionDistributions[i-1]
                mostLikely = max(g_dstr.keys(), key=(lambda k: g_dstr[k]))
                pg_dist = self.distancer.getDistance(pacmanPosition, mostLikely)
                for action in legal:
                    successorPosition = Actions.getSuccessor(pacmanPosition, action)
                    new_dist= self.distancer.getDistance(successorPosition, mostLikely)
                    if new_dist < pg_dist:
                        bestAction = action
        return bestAction
