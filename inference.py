...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        """
        # Initial State
        self.getJailPosition(): (1, 1)
        noisyDistance: 3
        emissionModel: {
            1: 0.16492146596858642, 2: 0.16753926701570682, 3: 0.33507853403141363, 4: 0.16753926701570682, 
            5: 0.08376963350785341, 6: 0.041884816753926704, 7: 0.020942408376963352, 8: 0.010471204188481676, 
            9: 0.005235602094240838, 10: 0.002617801047120419
        }
        pacmanPosition: (5, 2)
        self.beliefs: {
            (5, 4): 0.125, (3, 4): 0.125, (1, 4): 0.125, (7, 4): 0.125, (9, 4): 0.125, (5, 2): 0.125, (2, 4): 0.125, 
            (8, 4): 0.125
        }
        self.legalPositions: [(1, 4), (2, 4), (3, 4), (5, 2), (5, 4), (7, 4), (8, 4), (9, 4)]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        while count < self.numParticles:
            ind = count % len(self.legalPositions)
            self.pList.append(self.legalPositions[ind])
            count += 1
    ...
    def observe(self, observation, gameState):
        ...
        "*** YOUR CODE HERE ***"
        if noisyDistance is None:
            self.pList = [self.getJailPosition()] * self.numParticles
        else:
            beliefState = self.getBeliefDistribution()
            newBelief = util.Counter()
            for pos in beliefState:
                dist = util.manhattanDistance(pacmanPosition, pos)
                newBelief[pos] += emissionModel[dist] * beliefState[pos]
            if newBelief.totalCount() == 0:
                ...
                count = 0
                while count < self.numParticles:
                    newList.append(util.sample(newBelief))
                    count += 1
                self.pList = newList
    ...
    def elapseTime(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        allPossible = []
        for oldPos in self.pList:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            allPossible.append(util.sample(newPosDist))
        self.pList = allPossible
        # util.raiseNotDefined()
    def getBeliefDistribution(self):
        """
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        newList = list(itertools.product(self.legalPositions, repeat=self.numGhosts))
        random.shuffle(newList)
        self.pList = []
        for i in range(self.numParticles):
            self.pList.append(newList[i % len(newList)])
        ...
        for i, p in enumerate(self.pList):
            ...
            for j in range(0, self.numGhosts):
                if noisyDistances[j] is None:
                    ...
                    tempProb *= emissionModels[j][util.manhattanDistance(pacmanPosition, p[j])]
            allPossible[p] += tempProb
